...
def pdm_score(
    ...
    # RCP: Raw Centerline Progress (meters, no collision penalty)
    progress_raw = float(scorer._progress_raw[pred_idx])

    # RCR: Route Completion Ratio = min(1, agent_progress / expert_progress)
    expert_progress_raw = float(scorer._progress_raw[expert_idx])
    if expert_progress_raw > 0:
        route_completion_ratio = min(1.0, progress_raw / expert_progress_raw)
    else:
        route_completion_ratio = 1.0 if progress_raw == 0 else 0.0
