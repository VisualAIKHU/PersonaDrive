...
@dataclass
class PersonaDriveConfig:
    """Global TransFuser config."""

    # Text encoder backbone
    bert_backbone: str = "sentence-transformers/all-MiniLM-L6-v2"

    # PCMF (Persona-Conditioned Multi-Modal Fusion) slot sizes per source
    slots_per_source = {"agent_": 8, "ego": 2, "user": 2}
    purpose_div_weight: float = 1.0
    basis_weight: float = 1.0
    logits_class_weight: float = 10.0
    bert_hidden_size: int = 384
    num_purpose_class: int = 9 # UH_CL, UH_CM, UH_CH, UM_CL, UM_CM, UM_CH, UL_CL, UL_CM, UL_CH

    # ── Hierarchical conditioning (9-persona) ──
    neutral_persona_idx: int = 4          # UM_CM
    guide_weight: float = 1.0
    diversity_cross_weight: float = 0.2
    diversity_tau: float = 0.25
    diversity_margin_alpha: float = 1.0
    # alpha_c sigmoid-affine bound: alpha_c = sigmoid(.) * delta_c + gamma_c
    # default range [0.8, 1.2] (gamma_c=0.8, delta_c=0.4)
    gamma_c: float = 0.8
    delta_c: float = 0.4
    ...
    bkb_path: str = "navsim/pytorch_model.bin"
    plan_anchor_path: str = "navsim/kmeans_navsim_traj_20.npy"
