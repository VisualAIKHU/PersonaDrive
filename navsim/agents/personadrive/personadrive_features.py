...
class PersonaDriveFeatureBuilder(AbstractFeatureBuilder):
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        features["bert_feature"] = torch.tensor(agent_input.bert_feature)
        features["attention_mask"] = torch.tensor(agent_input.attention_mask)
...
class PersonaDriveTargetBuilder(AbstractTargetBuilder):
    ...
    def _compute_agent_targets(self, annotations: Annotations) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        def _xy_in_lidar(x: float, y: float, config: PersonaDriveConfig) -> bool:
