...
from navsim.visualization.bev import add_configured_bev_on_ax, add_trajectory_to_bev_ax, add_trajectory_to_bev_ax_
...
def plot_bev_with_agent_(scene: Scene, agent: AbstractAgent, uc: torch.tensor) -> Tuple[plt.Figure, plt.Axes]:
    """
    ...
    agent_trajectory = agent.compute_trajectory(scene.get_agent_input_uc(uc))
...
def plot_bev_with_trajectory(scene, trajectory) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent: navsim agent
    ...
    fig, ax = plt.subplots(1, 1, figsize=BEV_PLOT_CONFIG["figure_size"])
    add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
    add_trajectory_to_bev_ax(ax, trajectory, TRAJECTORY_CONFIG["human"])
    configure_bev_ax(ax)
    configure_ax(ax)

    return fig, ax

def plot_bev_with_gen_gts(scene, var_traj, conf) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent: navsim agent
    ...
    fig, ax = plt.subplots(1, 1, figsize=BEV_PLOT_CONFIG["figure_size"])
    add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
    add_trajectory_to_bev_ax_(ax, var_traj, conf)
    configure_bev_ax(ax)
    configure_ax(ax)

    return fig, ax

def plot_bev_with_agent_human(scene: Scene, agent: AbstractAgent) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent: navsim agent
...
def plot_bev_with_agent_pred(scene: Scene, agent: AbstractAgent) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent: navsim agent
...
def plot_bev_with_agent_pred_uc_(
    scene: Scene,
    agent_trajectory,
    ax: plt.Axes = None,
    intent: str = "normal"
    ...
    """
    Plots agent trajectory in birds-eye-view visualization
    """
    ...
    # Create a new figure if ax is None; otherwise draw on the given ax
    if ax is None:
        fig, ax = plt.subplots(1, 1, figsize=BEV_PLOT_CONFIG["figure_size"])
    else:
        fig = ax.figure
    ...
    add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
    add_trajectory_to_bev_ax(ax, agent_trajectory, TRAJECTORY_CONFIG[intent])
    configure_bev_ax(ax)
    configure_ax(ax)

    return fig, ax

def plot_bev_with_agent_uc_comb_(scene: Scene,
                                 agent_trajectory1: torch.tensor, agent_trajectory2: torch.tensor, 
                                 ax: plt.Axes = None, intent1: str = "normal", intent2: str = "normal") -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent: navsim agent
    ...
    # agent_trajectory1 = agent.compute_trajectory(scene.get_agent_input_uc(uc1))
    # agent_trajectory2 = agent.compute_trajectory(scene.get_agent_input_uc(uc2))
    ...
    if ax is None:
        ...
        fig = ax.figure
    add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
    add_trajectory_to_bev_ax(ax, agent_trajectory1, TRAJECTORY_CONFIG[intent1])
    add_trajectory_to_bev_ax(ax, agent_trajectory2, TRAJECTORY_CONFIG[intent2])
    configure_bev_ax(ax)
    configure_ax(ax)

    return fig, ax

def plot_bev_with_agent_uc_comb2_(scene: Scene,
                                 agent_trajectory1: torch.tensor, agent_trajectory2: torch.tensor, agent_trajectory3: torch.tensor, 
    ...
    if ax is None:
        ...
        fig = ax.figure
    add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
    add_trajectory_to_bev_ax(ax, agent_trajectory1, TRAJECTORY_CONFIG["emergency"])
    add_trajectory_to_bev_ax(ax, agent_trajectory2, TRAJECTORY_CONFIG["normal"])
    add_trajectory_to_bev_ax(ax, agent_trajectory3, TRAJECTORY_CONFIG["relaxed"])
    configure_bev_ax(ax)
    configure_ax(ax)

    return fig, ax

def plot_bev_with_agent_pred_uc(scene: Scene, agent: AbstractAgent, uc: torch.tensor) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent: navsim agent
...
def plot_cameras_frame_(scene: Scene, frame_idx: int) -> Tuple[plt.Figure, Any]:
    """
    ...
    fig, ax = plt.subplots(1, 3, figsize=CAMERAS_PLOT_CONFIG["figure_size"])

    add_camera_ax(ax[0], frame.cameras.cam_l0)
    add_camera_ax(ax[1], frame.cameras.cam_f0)
    add_camera_ax(ax[2], frame.cameras.cam_r0)
...
def plot_stitched_cameras_frame_(scene: Scene, frame_idx: int) -> Tuple[plt.Figure, Any]:
    """
    Plots 8x cameras and birds-eye-view visualization in 3x3 grid
    :param scene: navsim scene dataclass
    :param frame_idx: index of selected frame
